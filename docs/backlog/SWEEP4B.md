# Backlog: later ideas

**Status: not scheduled.** Ideas agreed with Zach that nobody is building yet. The old Growths and
grafting design that used to be here was dropped on 2026-09-18; grafting starts over, small, in
`docs/GRAFTING.md`.

## Grafting, later
- **Hives see through you.** Hives share a hive mind, so a surgeon with a grafted Hive eye is
  part of it. Now and then the Hives see what that surgeon sees, and come to where they're looking.
  This is the price of the graft. (Zach, 2026-09-18.)
- **The Sonographer's throat as a graft part.** A grafted surgeon gets the ping: the same wedge,
  their own throat glowing as they charge it. It replaces today's Echo ability. (Zach, 2026-09-18;
  the monster itself is `docs/SONOGRAPHER.md`.)
- **A solo surgical robot** at an OR table, so a solo player can be grafted. For now another player
  has to operate (Dr. Botsworth covers testing through the dev panel).
- A new level system, to replace brain points and one-graft-one-level alike.

## Dev panel
- Database: unlock all entries, reset.
- Send the ambulance now.
- Fog toggles: density, and the turnaround effect on or off.
- Give money and placebo bottles.

## Not designed yet
- **Abilities:** Puppet, Rise, ability side effects, moving abilities between slots.
- **Shop:** a gift shop, cosmetics.
- **From the original design:** two-person steps, networked physics and a shopping cart, voice
  chat, classes, progression.
- **Polish:** gown clipping, infection styles, balance, a flashlight for teammates.
- **Placebo ideas that were discussed but not specced:** silly stacking tiers, and a bottle
  rattle the Discharged can hear.
- **Open bugs** stay in `docs/KNOWN_ISSUES.md`. The Steam backend has never been tested against a
  real Steam client.
