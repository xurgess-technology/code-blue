...
def main():
    ...
    # The embedded export is an intermediate (git-ignored); the game gets the GLB with its textures as
    # separate PNGs (nn_glb_extern.py) so Godot's per-texture import settings (VRAM compression) stick.
    glb = os.path.join(os.path.dirname(HERE), 'night_nurse_embedded.glb')
