...
**In the game since 2026-09-14.** The model the game loads is
`assets/models/monsters/night_nurse/night_nurse.glb` (asset key `monster/night_nurse`) with its six
maps as separate PNGs in `textures/` beside it. This folder (it has a `.gdignore`, so Godot never
imports it) keeps everything needed to rebuild her: the Blender scripts, the `.blend`, the bake
inputs, and the look-dev renders and screenshots from the art test. How the game uses the model is in
`scripts/monsters/night_nurse_rig.gd` and `docs/CONTRACTS.md` ("The Night Nurse's model"). The art
test's Godot viewer was removed; `tools/monster_lab.tscn -- --shots --only=nurse_walk_flashlight,...`
takes the in-game pictures now. The rest of this file is the art test's write-up.
...
| Path | What |
|---|---|
| `../../assets/models/monsters/night_nurse/` | The game copy: skinned mesh, 2 materials, 3 animations, textures as separate PNGs |
| `godot_shots/` | Windowed 1280x720 screenshots from the art test's viewer (removed) |
| `blender_src/nn_glb_extern.py` | Splits the exported GLB's embedded images into PNG files and writes the game copy |
...
godot --headless --fixed-fps 60 --path . tools/monster_lab.tscn                          # the Nurse's checks
godot --path . --resolution 1280x720 tools/monster_lab.tscn -- --shots --only=nurse_walk_flashlight,nurse_frozen_watched,nurse_face
```

`nn_build.py` exports `art/night_nurse/night_nurse_embedded.glb` (not committed), then
