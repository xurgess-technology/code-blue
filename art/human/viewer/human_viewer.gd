...
func _shot_lineup(lit: bool) -> void:
	...
	for i in VARIANTS.size():
		var lane := -1.9 + i * 0.76
		spawn(VARIANTS[i], cor(x, lane), Vector3(-1, 0, 0), "Idle", 0.3 + i * 0.5)
	if lit:
		set_light(1, true, 1.1)
		set_light(2, true, 0.8)
		look_from(cor(x - 4.6, 0.0) + Vector3.UP * 1.45, cor(x, 0.0) + Vector3.UP * 1.0, 60.0, false)
	else:
		look_from(cor(x - 4.2, 0.1) + Vector3.UP * C.EYE_H, cor(x, 0.0) + Vector3.UP * 1.1, 70.0, true)
...
func _shot_tinted() -> void:
	...
	for i in 4:
		var h := spawn(v[i], cor(x, -1.2 + i * 0.8), Vector3(-1, 0, -0.15 + i * 0.1).normalized(), "Idle", i * 0.7)
	...
	set_light(1, true, 1.1)
	set_light(2, true, 0.8)
	look_from(cor(x - 3.8, 0.0) + Vector3.UP * 1.5, cor(x, -0.1) + Vector3.UP * 1.1, 62.0, false)


func _shot_walk() -> void:
	var pos := cor(20.0, 0.1)
	spawn("surgeon_c", pos, Vector3(-1, 0, 0.15).normalized(), "Walk", 0.35)
	spawn("paramedic_a", cor(21.6, -0.8), Vector3(-1, 0, 0), "Jog", 0.2)
	set_light(1, true, 0.4)
	look_from(cor(16.5, -0.3) + Vector3.UP * C.EYE_H, pos + Vector3.UP * 1.2, 70.0, true)
...
func _shot_kenney() -> void:
	var pos_new := cor(20.0, -0.5)
	var pos_old := cor(20.0, 0.5)
	...
	set_light(2, true, 0.7)
	look_from(cor(16.6, 0.0) + Vector3.UP * 1.3, cor(20.0, 0.0) + Vector3.UP * 1.0, 55.0, false)
...
func _shot_or_amputation() -> void:
	...
	var c := s.global_position if s != null else or_pos + Vector3.UP
	look_from(c + Vector3(-0.35, 0.85, 0.55), c + Vector3(-0.3, 0, 0), 60.0, false)
...
func _shot_carry() -> void:
	...
	set_light(1, true, 0.8)
	set_light(2, true, 0.6)
	look_from(cor(16.8, 1.8) + Vector3.UP * 1.5, pos + Vector3.UP * 1.1, 60.0, false)
